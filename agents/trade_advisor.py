...
- Stop loss must be beyond a structural level, not arbitrary"""
    ...
    try:
        raw = None
        # Generate with Gemini 2.5 Flash first (sound levels, correct RR)
        try:
            raw = _gemini_generate(prompt)
            print("[trade_advisor] signal generated by Gemini 2.5 Flash")
        except Exception as gem_err:
            print(f"[trade_advisor] Gemini gen failed ({str(gem_err)[:40]}) — Llama fallback")
            try:
